...
def Dictionnaire(Amax, bits):
    step = 2*Amax / (2**bits)
    print(f"le step est {step}")
    return np.arange(-Amax+step/2, Amax-step/2 + step, step)
...
def convertirDico(quantification, dictionnaire):
    bits = int(math.log2(len(dictionnaire))) + 1
    listenb = np.arange(-len(dictionnaire)/2, (len(dictionnaire)-1)/2, 1, int)
    print(listenb)
    converti = [toBin(k, bits) for k in listenb]
    print(converti)
    final = []
    for k in range(len(quantification)):
        for j in range(len(dictionnaire)):
            if(quantification[k] == dictionnaire[j]):
                final.append(converti[j])
    f2 = []
    for k in range(len(final)):
        for j in range(len(final[0])):
            f2.append(int(final[k][j]))
    return f2
